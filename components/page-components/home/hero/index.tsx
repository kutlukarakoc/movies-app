...
const Hero: React.FC = () => {
  return (
    <section className='w-full h-[450px] md:h-[625px] lg:h-[725px] relative mb-28'>
      <Image src={heroMockup.backdropImage} alt={heroMockup.title} className='object-cover' fill sizes='100vw' quality={100} />
      <div className='w-full absolute z-10 text-center md:text-left bottom-5 md:bottom-20 left-1/2 -translate-x-1/2 md:-translate-x-0 md:left-14 max-w-[85%] md:max-w-[75%] lg:max-w-screen-md'>
        ...
        <Link href={`/movie/${heroMockup.id}`} className='bg-accent text-secondary flex items-center justify-center text-sm md:text-lg w-36 md:w-44 h-10 md:h-14 rounded-sm hover:bg-accent-light'>
          More İnformation
        </Link>
      </div>
    </section>
